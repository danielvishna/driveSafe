        ...
        const response = await axiosInstance.post(
            ...
            },
            {
                validateStatus: function (status: number) {
                    return status < STATUS_SERVERERROR; // Allows 400 status to be handled in the try block
                },
            }
        );

        if (response.status === STATUS_OK && typeof response.data === 'object' && response.data !== null) {
...
const createPayload = (
    ...
    return {
        ...
        associated_accounts: account_type === 'Virtual Account' ? associated_accounts : [],
    };
};
...
export const signup = async (
    ...
    const url = accountType === 'Child Account' ? '/create_sub_account' : '/signup';
...
export const getAccountDetails = async (
    ...
    try {
        ...
        return { response: null, errorMessage: "There's been an error. Please try again later" };
    }
};
...
const processTransactions = (transactions: TransactionGroup, transactionsType: RequestType): TransactionDetails[] => {
    ...
    const sortedTransactions = allTransactions.sort((a, b) => b.last_timestamp - a.last_timestamp);
...
export const getLastTransactions = async (
    ...
    if (response.errorMessage === '' && response.response !== null) {
        return {
            response: processTransactions(response.response, RequestType.All).slice(0, 5),
            errorMessage: '',
        };
    } else {
        return { response: null, errorMessage: response.errorMessage };
    ...
    try {
        ...
        if (axios.isAxiosError(error)) {
            ...
            if (axiosError.response?.data.response) {
                return { success: false, message: String(axiosError.response.data?.response) };
            }
        }
    }
    return { success: false, message: 'Failed to load balance. Please try again later' };
};
export const performTransaction = async (
    ...
    if (operation === 'transfer') {
        ...
        return { success: false, message: `Failed to ${operation}. Please try again later` };
    }
    try {
...
export const requestApproval = async (
    ...
    return { success: false, message: `Failed to request approval ${operation}. Please try again later` };
};
...
export const denyTransaction = async (txn: TransactionDetails): Promise<{ success: boolean; message: string }> => {
    try {
    ...
    return { success: false, message: 'Failed to deny transaction. Please try again later' };
};

export const approveTransaction = async (txn: TransactionDetails): Promise<{ success: boolean; message: string }> => {
    try {
    ...
    return { success: false, message: 'Failed to Approve transaction. Please try again later' };
};
...
export const requestHigherApproval = async (
    ...
    try {
        ...
        if (response.status === 200) {
            return { success: true, message: 'Request higher approval successfully!' };
        }
    } catch (error) {
        if (axios.isAxiosError(error)) {
            const axiosError = error as AxiosError<AxiosErrorResponse>;
    ...
    return { success: false, message: 'Failed to request higher approval . Please try again later' };
};
